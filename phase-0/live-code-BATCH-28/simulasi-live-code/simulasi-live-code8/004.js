...
function simpleCardsFusion (str) {
  ...
  let [strArr, result ]=[ str.split("+"), ''];
  let [kartu2, kartu1] = strArr;
  for( {name, card1, card2} of fusionCards){
    if( card2 == kartu2 && card1 === kartu1 ){
      result = name;
    } else if( card2 == kartu1 && card1 === kartu2){
      result = name;
    }
  }
  return !result ? 'Fusion failed' : result;
}
